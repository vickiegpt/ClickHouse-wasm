...
if [ ! -x "${SCRIPT_DIR}/synthetic_hft" ]; then
    echo "Error: synthetic_hft not built"
    echo "Build it first with: make -f Makefile.v2 synthetic_hft"
    exit 1
fi

export LD_PRELOAD="$ZEROCOPY_LIB"
export ZEROCOPY_THRESHOLD="${ZEROCOPY_THRESHOLD:-1}"
export ZEROCOPY_QUEUE_DEPTH="${ZEROCOPY_QUEUE_DEPTH:-128}"
export ZEROCOPY_DEBUG="${ZEROCOPY_DEBUG:-0}"

echo "========================================"
echo "Synthetic HFT + zerocopy v2"
echo "========================================"
